...
```{r}
forward <- ols_step_forward_p(mod, details = TRUE)
plot(forward)
```
...
```{r}
fwreg <- ols_step_both_p(mod, details = TRUE)
plot(fwreg)
```
...
```{r}
best_model <- lm(AAPL.Vol ~  VXAZNCLS + AMZN.Close + (AMZN.Close3) + GSPC.Close + (GSPC.Close3)  + (AMZN.PVol2)  + (AMZN.PVolGOOG.PVol4))
#summary(best_model)
#ols_step_backward_aic(best_model)

date_model <- lm(AAPL.Vol ~  Date + Date2 + Date3 + VXAZNCLS + (GSPC.Close3)  + (AMZN.PVol2)  + (AMZN.PVolGOOG.PVol4))
#summary(date_model)
