...
corr_matrix = cbind(VXAPLCLS,VXAZNCLS,AMZN.Close,GSPC.Close,GSPC.Ret,AAPL.PVol,AMZN.PVol,GOOG.PVol,GSPC.PVol)
...
```{r}
summary(lm(AAPL.Vol ~ vxaplcls+vxazncls+amznclose+gspcclose+gspcret+aaplpvol+amznpvol+googpvol+gspcpvol + (googpvol*gspcpvol*amznpvol*aaplpvol)))

###uncenter

#c <- mean(VXAPLCLS)+mean(VXAZNCLS)+mean(AMZN.Close)+mean(GSPC.Ret)+mean(AMZN.PVol)+
#lm(AAPL.Vol ~ VXAPLCLS+VXAZNCLS+AMZN.Close+GSPC.Ret+AMZN.PVol+ (GOOG.PVol+mean(GOOG.PVol)*GSPC.PVol+mean(GSPC.PVol)*AMZN.PVol+mean(AMZN.PVol)*AAPL.PVol+mean(AAPL.PVol)))
...
best_model <- lm(AAPL.Vol ~  VXAZNCLS  + AMZN.Close + (AMZN.Close2) + GSPC.Close + (GSPC.Close2)  + (AMZN.PVol2)  + (AMZN.PVol*GOOG.PVol))
summary(best_model)
plot(best_model)
